...
app.put('/api/atletas/:id', async (req, res) => {
    const id = parseInt(String(req.params.id));
    const { nome, cpf, dataNasc } = req.body;
    try {
...
// --- ELENCO ---
app.get('/api/equipes/:id/elenco', async (req, res) => {
...
// --- PARTICIPAÇÕES ---
app.get('/api/campeonatos/:id/equipes', async (req, res) => {
...
// --- CLASSIFICAÇÃO ---
app.get('/api/campeonatos/:id/classificacao', async (req, res) => {
    const campeonatoId = parseInt(String(req.params.id));
    ...
    try {
        ...
        jogos.forEach(j => {
            if (j.mandanteId !== null && j.visitanteId !== null) {
                ...
                if (tabela[mId] && tabela[vId]) {
                    tabela[mId].j++;
                    tabela[vId].j++;
                    tabela[mId].gp += j.golsMandante;
                    tabela[mId].gc += j.golsVisitante;
                    tabela[vId].gp += j.golsVisitante;
                    tabela[vId].gc += j.golsMandante;
                    if (j.golsMandante > j.golsVisitante) {
                        tabela[mId].p += 3;
                        tabela[mId].v++;
                        tabela[vId].d++;
                    }
                    else if (j.golsMandante < j.golsVisitante) {
                        tabela[vId].p += 3;
                        tabela[vId].v++;
                        tabela[mId].d++;
                    }
        ...
        resultado.sort((a, b) => (b.p - a.p) || (b.v - a.v) || (b.sg - a.sg) || (b.gp - a.gp));
        res.json(resultado);
    }
    catch (e) {
...
// --- RODADAS ---
app.get('/api/campeonatos/:id/rodadas', async (req, res) => {
    const campeonatoId = parseInt(String(req.params.id));
    try {
...
app.get('/api/jogos/:id/detalhes', async (req, res) => {
    ...
    try {
        ...
        const buscarElenco = async (equipeId) => {
            const vinculos = await prisma.vinculo.findMany({
                where: { equipeId },
                include: { atleta: true }
            });
        ...
        const elencoMandante = jogo.mandanteId !== null ? await buscarElenco(jogo.mandanteId) : [];
        const elencoVisitante = jogo.visitanteId !== null ? await buscarElenco(jogo.visitanteId) : [];
...
app.post('/api/jogos/finalizar', async (req, res) => {
...
app.post('/api/campeonatos/:id/gerar-chaveamento', async (req, res) => {
    const campeonatoId = parseInt(String(req.params.id));
    try {
        ...
        for (const chave of chavesUnicas) {
            ...
            jogosGrupo.forEach(j => {
                if (j.mandanteId !== null && j.visitanteId !== null) {
                    const mId = j.mandanteId;
                    const vId = j.visitanteId;
                    if (tabela[mId] && tabela[vId] && j.chave === chave) {
                        tabela[mId].gp += j.golsMandante;
                        tabela[mId].gc += j.golsVisitante;
                        tabela[vId].gp += j.golsVisitante;
                        tabela[vId].gc += j.golsMandante;
...
// --- ESTATÍSTICAS ---
app.get('/api/campeonatos/:id/estatisticas', async (req, res) => {
    const id = parseInt(String(req.params.id));
    try {
        ...
        const disciplina = Object.values(disciplinaMap).sort((a, b) => b.peso - a.peso);
        ...
        console.log(`[API STATS DIST] Enviando ${artilharia.length} registros de artilharia para o camp ${id}`);
...
app.get('/api/admin/prefeituras', checkSuperAdmin, async (req, res) => {
    try {
...
app.post('/api/admin/prefeituras', checkSuperAdmin, async (req, res) => {
...
app.put('/api/admin/prefeituras/:id', checkSuperAdmin, async (req, res) => {
    const id = parseInt(String(req.params.id));
    const { nome, slug, logoUrl, corPrimaria, corSecundaria } = req.body;
    try {
...
app.delete('/api/admin/prefeituras/:id', checkSuperAdmin, async (req, res) => {
    const id = parseInt(String(req.params.id));
    try {
...
app.get('/api/admin/usuarios', checkSuperAdmin, async (req, res) => {
    try {
...
app.post('/api/admin/usuarios', checkSuperAdmin, async (req, res) => {
...
app.put('/api/admin/usuarios/:id', checkSuperAdmin, async (req, res) => {
    const id = parseInt(String(req.params.id));
    const { email, senha, prefeituraId } = req.body;
    try {
...
app.delete('/api/admin/usuarios/:id', checkSuperAdmin, async (req, res) => {
    const id = parseInt(String(req.params.id));
    try {
