...
export const getCategorias = async (req: Request, res: Response) => {
    ...
    if (!prefeituraId) return res.status(400).json({ error: "Prefeitura ID é obrigatório" });
...
export const createCategoria = async (req: Request, res: Response) => {
    ...
    if (!nome || !prefeituraId) return res.status(400).json({ error: "Nome e Prefeitura ID são obrigatórios" });
